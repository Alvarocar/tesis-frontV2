...
const Procesos: React.FC<Props> = ({ params }) => {
  const { isLoading, data } = useSWR(isValidSlug(params.id) ? { id: params.id, name: "procesos" } : undefined, vacantRepository.getVacanciesByApplications.bind(vacantRepository))
